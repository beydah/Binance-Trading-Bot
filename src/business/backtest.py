...
def DCA_ALGORITHM(LEFT_SMYBOL, RIGHT_SYMBOL, CANDLE_PERIOD, WALLET):
    ...
    for i in range(len(closePrice)):
        if LIB.PD.isna(sma25[i]) is False:
            signal = INDICATOR.DCA_SIGNAL(closePrice[i - 2], sma25[i - 2], closePrice[i - 1], sma25[i - 1])
...
def MIX_ALGORITHM(LEFT_SMYBOL, RIGHT_SYMBOL, CANDLE_PERIOD, WALLET):
    ...
    for i in range(len(closePrice)):
        if not any(LIB.PD.isna([stochRSI_K[i], RSI[i], sma50[i], sma25[i]])):
            signalBuy = 0
            signalSell = 0
            if INDICATOR.DCA_SIGNAL(closePrice[i-2], sma25[i-2], closePrice[i-1], sma25[i-1]) == 1: signalBuy += 1
            elif INDICATOR.DCA_SIGNAL(closePrice[i-2], sma25[i-2], closePrice[i-1], sma25[i-1]) == -1: signalSell += 1
            if INDICATOR.GOLDENCROSS_SIGNAL(sma50[i-2], sma200[i-2], sma50[i-1], sma200[i-1]) == 1: signalBuy += 1
            elif INDICATOR.GOLDENCROSS_SIGNAL(sma50[i-2], sma200[i-2], sma50[i-1], sma200[i-1]) == -1: signalSell += 1
            if INDICATOR.STOCHRSI_SIGNAL(stochRSI_K[i-1]) == 1: signalBuy += 1
            elif INDICATOR.STOCHRSI_SIGNAL(stochRSI_K[i-1]) == -1: signalSell += 1
            if INDICATOR.RSI_SIGNAL(RSI[i - 1]) == 1: signalBuy += 1
            elif INDICATOR.RSI_SIGNAL(RSI[i - 1]) == -1: signalSell += 1
            if signalBuy >= signalLimit and WALLET != 0:
                print("####################################################")
                totalCoin = CALCULATE.TEST_BUY(WALLET, closePrice[i])
                WALLET = 0
                buyNum += 1
                print(f"{totalCoin} - {LEFT_SMYBOL} - BUY - {openTime[i]}")
            if signalSell >= signalLimit and totalCoin != 0:
                print(f"{totalCoin} - {LEFT_SMYBOL} - SELL - {openTime[i]}")
                WALLET = CALCULATE.TEST_SELL(totalCoin, closePrice[i])
                totalCoin = 0
                sellNum += 1
